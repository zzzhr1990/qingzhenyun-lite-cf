...
void RemoteUserFilePanel::OnThreadEvent(wxThreadEvent& event)
{
    ...
    if(eventType == USER_REMOTE_FILE_PAGE_DATA){
        ...
        for(const auto& i : list){//std::cout << i.at(U("name")) << std::endl;
            ...
            data.push_back(i.at(U("name")).as_string());
            data.push_back(ConvertSizeToDisplay(i.at(U("size")).as_number().to_int64()));
            data.push_back(i.at(U("mime")).as_string());
            time_t time = i.at(U("atime")).as_number().to_int64() / 1000;
            //current.
            data.push_back(ConvertTimeToDisplay(time));
            data.push_back(i.at(U("path")).as_string());
            data.push_back(i.at(U("type")).as_integer());
