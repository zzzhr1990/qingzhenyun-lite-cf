...
void RemoteDownloadTaskPanel::OnThreadEvent(wxThreadEvent& event)
{
    ...
    if(eventType == USER_REMOTE_TASK_PAGE_DATA){
        ...
        for(const auto& i : list){
            ...
            data.push_back(i.at(U("name")).as_string()); //0: name
            data.push_back(ConvertSizeToDisplay(i.at(U("size")).as_number().to_int64())); // 1:size
            ...
            data.push_back(progress); // 2:progress

            int status = i.at(U("status")).as_integer();

            if(status < 0){
                data.push_back(wxString::Format(_T("Error %d"),i.at(U("errorCode")).as_integer()));
            }else{
                if(status == 90){
                    data.push_back(_T("Copying"));
                } else if(status == 100){
                    data.push_back(_T("Finish"));
                }else{
                    data.push_back(_T("Downloading"));
                }
            } // 3:status
            ...
            //current.
            data.push_back(ConvertTimeToDisplay(time)); // 4 time
            long userId = i.at(U("userId")).as_number().to_int64();
            data.push_back(userId); // 5 userId
            data.push_back(i.at(U("taskId")).as_string()); //6 task id
