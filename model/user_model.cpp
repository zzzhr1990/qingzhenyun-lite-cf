...
pplx::task<ResponseEntity> UserModel::check_login(const std::string &value, const std::string& password) {
	...
	return CommonApi::Instance().PostData(U("/v1/user/login"), json_v);
...
void UserModel::StartUserCheckLoop(wxWindow* handler, const int& eventId) {
	// start a thread
...
bool UserModel::CheckUserLogin(wxWindow* handler, const int &event_id) {
	if (IsUserLogin()) {
		// chk
		GetUserInfo().then([&, handler](ResponseEntity v) {
			...
			if (v.success) {
				...
				if (v.status > 400 && v.status < 500) {
					wxThreadEvent event(wxEVT_THREAD);
					event.SetId(event_id);
					event.SetInt(NEED_NEED_RELOGIN_RESPONSE);
					// using namespace chrono;
					auto ts = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
					event.SetTimestamp(ts);
					// event.Clone();
...
pplx::task<ResponseEntity> UserModel::GetUserInfo() {
	using namespace web;
	return CommonApi::Instance().PostData(U("/v1/user/info"), web::json::value::object(false));
